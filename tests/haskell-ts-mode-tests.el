...
not the migration check it started as."
  (cl-loop
   ...
   (let ((node (haskell-ts--text-node-at pos))
         (r (haskell-ts--region-at pos)))
     (if node
         (progn
           (should (eq (haskell-ts--region-kind r)
                       (pcase (treesit-node-type node)
                         ("string" 'string)
                         ("haddock" 'haddock)
                         (_ 'comment))))
           (should (= (haskell-ts--region-beg r) (treesit-node-start node)))
           (should (= (haskell-ts--region-end r) (treesit-node-end node))))
       (should (eq (haskell-ts--region-kind r) 'code))
       (should (= (haskell-ts--code-paragraph-limit 1)
                  (min (haskell-ts--code-blank-line-limit 1)
                       (haskell-ts--region-end r))))
       (should (= (haskell-ts--code-paragraph-limit -1)
...
-- Hello. This is a sentence.\nmain :: IO ()\nmain = putStrLn greeting\n"
             ...
             "f = id\n\ng = id\n"
             ;; Blank line between a block comment and a Haddock comment.
             "{-------------------------------------------------------------------------------\n\
  Section\n\
-------------------------------------------------------------------------------}\n\n\
-- | Haddock\ndata Data\n"))
