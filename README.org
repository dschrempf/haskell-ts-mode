...
=type_synomym= → =type_synonym=).  The font-lock queries rely on the
corrected names, so the official grammar does *not* work with this mode:
it fails silently — usually as missing or wrong highlighting rather than
an error.
