...
tree that Emacs's built-in =treesit= keeps up to date.
...
- A Haskell grammar built from @tek's fork, see [[#grammar][Installing the grammar]].
...
=haskell-ts-mode= targets @tek's grammar, which fixes node type names that the
official grammar gets wrong (=type_synomym= → =type_synonym=, for example).
The font lock queries rely on the corrected names, so the official grammar does
...
repository, and neither @tek's repository nor mine ships a generated parser
(=src/parser.c=) or a release branch, so the parser has to be produced with
